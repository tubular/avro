                            ...
                            const char *field_name)
{
    ...
    while (avro_file_reader_read_value(file, &writer_value) == 0) {
		fprintf(stderr, "stepped:a %s\n", field_name);
        avro_value_t  field;
        int32_t  value;
		check_i(avro_value_get_by_name(&reader_value, field_name, &field, NULL));
		if(strcmp(field_name, "b") == 0)
		{
			fprintf(stderr, "comapred:b\n");
        	check_i(avro_value_get_int(&field, &value));
			fprintf(stderr, "  %s: %" PRId32 "\n", field_name, value);
		}
		if(strcmp(field_name, "c") == 0)
		{
			fprintf(stderr, "comapred:c\n");
			const char  *actual_str = NULL;
			size_t  actual_size = 0;
        	avro_value_get_string(&field, &actual_str, &actual_size);
			fprintf(stderr, "The string  %s: %s \n", field_name, actual_str);
...
{
	write_data("test_file.dat");
	//read_with_schema_resolution("test_file.dat", READER_SCHEMA_B, "b");
	read_with_schema_resolution("test_file.dat", READER_SCHEMA_B, "c");
}
